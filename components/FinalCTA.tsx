...
export default function FinalCTA() {
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute -top-32 right-[-10%] h-96 w-96 rounded-full bg-gold/10 blur-3xl"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -bottom-32 left-[-10%] h-96 w-96 rounded-full bg-forest/10 blur-3xl"
      />
      ...
      <div className="relative grid gap-12 lg:grid-cols-[1fr_1.05fr] lg:items-center lg:gap-16">
        <div className="reveal">
          ...
          <h2 className="h-display mt-5 text-[2.1rem] leading-[1.06] text-ink sm:text-[2.6rem] lg:text-[3.3rem]">
            ...
            <br />
            <span className="text-forest">No charge, no catch.</span>
          </h2>
          ...
          <ul className="mt-8 space-y-3">
            {reassurance.map((r) => (
              ...
              >
                <span
                  aria-hidden
                  className="flex h-6 w-6 items-center justify-center rounded-full border border-forest/30 text-[11px] text-forest"
                >
                  ✓
                </span>
                {r}
